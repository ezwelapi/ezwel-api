...
(function( $ ){
	$.extend({
		 apiConfig: {
			cont: {
				// $.apiConfig.cont.firstData
				firstData: "Operating ENV XML"
				// $.apiConfig.cont.ctxp
				,ctxp: "/API1.0"
				,txURL: {
					 saveXML: $.gf_cont.reqNamespace + "/manager/saveXML"	// $.apiConfig.cont.txURL.saveXML
					,findXML: $.gf_cont.reqNamespace + "/manager/findXML"	// $.apiConfig.cont.txURL.findXML
					,applyXML: $.gf_cont.reqNamespace + "/manager/applyXML"	// $.apiConfig.cont.txURL.applyXML
					,storeAllList: $.gf_cont.reqNamespace + "/manager/storeAllList"	// $.apiConfig.cont.txURL.storeAllList
					,applyList: $.gf_cont.reqNamespace + "/manager/applyList"	// $.apiConfig.cont.txURL.applyList
					,applyView: $.gf_cont.reqNamespace + "/manager/applyView"	// $.apiConfig.cont.txURL.applyView
				}
			},
			provider: {
				 // $.apiConfig.provider.fileCache
				 fileCache: {}
				 // $.apiConfig.provider.docs
				,docs: {}
				// $.apiConfig.provider.mainSplit
				,mainSplit: {}
				// $.apiConfig.provider.optionsPanel
				,optionsPanel: null
				// $.apiConfig.provider.doclist
				,doclist: null
			}
			,util: {
				// $.apiConfig.util.errorMessage
				...
				// $.apiConfig.util.getUserId()
				getUserId: function() {
					var inParam = $.gf_func.getParamObject();
					return (inParam.userId ? inParam.userId : "tester");
				}
				// $.apiConfig.util.addDocument()
				,addDocument: function( module, storeFileName ) {
					
					//START. set doclist
					$.apiConfig.tx.storeAllList( function(response) {
						
						//초기화
						$.apiConfig.provider.docs = {};
						
						$.each(response.fileList, function(i, data){
							
							 //$.logger.warn(">>> saveXML after", data);
							
							if( data.fileType == "D" ) {
								$.logger.debug("data.name = " + data.name);
								$.apiConfig.provider.docs[data.name] = {};
							}
							else if( data.fileType == "F" ) {
								$.logger.debug("data.prntName = " + data.prntName + ", data.name = " + data.name);
								$.apiConfig.provider.docs[data.prntName][data.name] = data;
							}
						});
						
						//문서 목록 exports
						for(var key in $.apiConfig.provider.docs) {
							module.exports[key] = $.apiConfig.provider.doclist.sort($.apiConfig.provider.doclist.prepareDocList($.apiConfig.provider.docs[key]));
						}
						
						//문서 그룹 목록 exports
						module.exports.all = {};
						for(var key in $.apiConfig.provider.docs) {
							module.exports.all[key] = module.exports[key];
						}
						
						if( $.apiConfig.provider.optionsPanel != null ) {
							
							//$.logger.debug("#$.apiConfig.provider.optionsPanel 1", $.apiConfig.provider.optionsPanel.options.doc.items);
							
							$.apiConfig.provider.optionsPanel.options.doc.items = module.exports.all;

							//$.logger.debug("#$.apiConfig.provider.optionsPanel 2", $.apiConfig.provider.optionsPanel.options.doc.items);
							
							//$.logger.debug("#- module.exports.all", module.exports.all);
							
							$("#controls #-doc").empty();
							
							var fileData = null;
							for(var key in module.exports.all) {

								$("#controls #-doc").append($('<optgroup>', {
									label: key
								}));
								
								fileData = module.exports.all[key];
								//$.logger.debug("fileData : ", fileData);

								$.each(fileData, function(idx, file){
									$("#controls #-doc optgroup[label='"+key+"']").append($('<option>', {
										value: file.name,
										text : file.name
									}));
								});
							...
							$("#controls #-doc").val( storeFileName );
						}
					});
					//END. set doclist
				}
			...
			,tx: {
				// $.apiConfig.tx.storeAllList( callback )
				storeAllList: function( callback ) {
					$.logger.debug("[START] $.apiConfig.tx.storeAllList", arguments);
					
					var url = $.apiConfig.cont.txURL.storeAllList;
					// 1. 유저 폴더 목록 조회
					$.gf_tx.ajax( url, null
					,{
						fn_success: function( response ) {
							...
							if(response.code !== "1000") {
								// error
								$.apiConfig.util.errorMessage(url, response);
							}
							else {
								$.logger.debug("#storeAllList-response", response);
								
								$.gf_func.callback(callback, [response]);
							}
						}
					});
				}
				// $.apiConfig.tx.saveXML( userId, xmlCont, callback )
				,saveXML: function( userId, xmlCont, callback ) {

					$.gf_tx.ajax( $.apiConfig.cont.txURL.saveXML, { 
			        	 storeFileDir: userId
			        	,xmlCont: xmlCont
			        }
					,{
						fn_success: function( response ) {
							...
							// 저장 실패
							if(response.code !== "1000") {
								$.apiConfig.util.errorMessage( $.apiConfig.cont.txURL.saveXML, response );
							}
							// 저장 성공
							else {
								$.logger.debug("#saveXML-response", response);
				...
				// $.apiConfig.tx.applyXML( callback )
				,applyXML: function( userId, fileName, callback ) {

					$.gf_tx.ajax( $.apiConfig.cont.txURL.applyXML, { 
			        	 storeFileDir: userId
					...
					,{
						fn_success: function( response ) {
							...
							// 저장 실패
							if(response.code !== "1000") {
								$.apiConfig.util.errorMessage( $.apiConfig.cont.txURL.applyXML, response );
							}
							// 저장 성공
							else {
								$.logger.debug("#applyXML-response", response);
				...
				// $.apiConfig.tx.applyList( params, callback )
				,applyList: function( params, callback ) {
					$.logger.debug("[START] $.apiConfig.tx.applyList", arguments);
					
					var url = $.apiConfig.cont.txURL.applyList;
					
					$.gf_tx.ajax( url, params
					,{
						fn_success: function( response ) {
							...
							if(response.code !== "1000") {
								...
								$.logger.debug("#applyList-response", response);
				...
				// $.apiConfig.tx.applyView( params, callback )
				,applyView: function( params, callback ) {
					
					var url = $.apiConfig.cont.txURL.applyView;
							...
							,{
								fn_success: function( response ) {
									...
									if(response.code !== "1000") {
										...
										$.logger.debug("#applyView-response", response);
			...
			,define: function( storeFile ) {
				...
				define("ace/ext/rtl",["require","exports","module","ace/lib/dom","ace/lib/lang","ace/editor","ace/config"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] => ace/ext/rtl", arguments);
						...
						require("../config").defineOptions(Editor.prototype, "editor", {
						    rtlText: {
						        set: function(val) {
						        	$.logger.debug("## editor rtlText set ##");
						...
						function onChangeSelection(e, editor) {
							$.logger.debug("## onChangeSelection ##");
							$.logger.debug(arguments);
						...
						function onCommandEmitted(commadEvent) {
							$.logger.debug("## onCommandEmitted ##", arguments);
						...
						function onChange(delta, session) {
							$.logger.debug("## onChange ##", arguments);
						...
						function updateLineDirection(e, renderer) {
							$.logger.debug("## updateLineDirection ##", arguments);
						...
						function clearTextLayer(renderer) {
							$.logger.debug("## clearTextLayer ##", arguments);
				...
				define("ezc-api/inline_editor",["require","exports","module","ace/line_widgets","ace/editor","ace/virtual_renderer","ace/lib/dom","ace/commands/default_commands"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] => ezc-api/inline_editor", arguments);
						...
						require("ace/commands/default_commands").commands.push({
						    ...
						    bindKey: "F3",
						    exec: function(editor) {
						    	$.logger.debug("## default_commands exec ##");
						    	$.logger.debug(arguments);
				...
				define("ace/test/asyncjs/assert",["require","exports","module","ace/lib/oop"], 
					function(require, exports, module) {
						$.logger.warn("[define] => ace/test/asyncjs/assert", arguments);
						...
						$.logger.debug("## AssertionError ##", arguments);
						...
						assert.AssertionError.prototype.toString = function() {
							$.logger.debug("## assert.AssertionError.prototype.toString ##", arguments);
						...
						function _deepEqual(actual, expected) {
							$.logger.debug("## _deepEqual ##", arguments);
						...
						function objEquiv(a, b) {
						  ...
						  try {
						    var ka = Object.keys(a),
						        kb = Object.keys(b),
						        key, i;
						  } catch (e) {// happens when one is a string literal and the
										// other isn't
						    return false;
						  }
				...
				define("ace/test/asyncjs/async",["require","exports","module"], 
					function(require, exports, module) {
						$.logger.warn("[define] => ace/test/asyncjs/async", arguments);
				...
				define("ace/test/mockrenderer",["require","exports","module"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] => ace/test/mockrenderer", arguments);
				...
				define("ezc-api/dev_util",["require","exports","module","ace/lib/dom","ace/lib/event","ace/range","ace/edit_session","ace/undomanager","ace/lib/oop","ace/lib/dom","ace/range","ace/editor","ace/test/asyncjs/assert","ace/test/asyncjs/async","ace/undomanager","ace/edit_session","ace/test/mockrenderer","ace/lib/event_emitter"], 
					function(require, exports, module) {
						$.logger.warn("[define] => ezc-api/dev_util", arguments);
						...
						function warn() {
						    ...
						    s = s.split("\n");
						    if (s[1] == "Error") s.shift(); // remove error description
															// on chrome
						    s.shift(); // remove warn
						...
						exports.closeLogView = function() {
							$.logger.debug("## exports.closeLogView ##");
							$.logger.debug(arguments);
						...
						function onBeforeEndOperation() {
							$.logger.debug("## onBeforeEndOperation ##");
							$.logger.debug(arguments);
				...
				define("ace/ext/modelist",["require","exports","module"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] => ace/ext/modelist", arguments);
						
						var modes = [];
						function getModeForPath(path, fileName) {
							$.logger.debug("#getModeForPath", arguments);
							if(fileName) {
								path = fileName;
							}
				...
				define("ezc-api/file_drop",["require","exports","module","ace/config","ace/lib/event","ace/ext/modelist","ace/editor"], 
					function(require, exports, module) {
						$.logger.warn("[define] => ezc-api/file_drop", arguments);
				...
				define("ace/theme/textmate",["require","exports","module","ace/lib/dom"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/theme/textmate", arguments);
				...
				define("ace/ext/whitespace",["require","exports","module","ace/lib/lang"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/ext/whitespace", arguments);
				...
				define("ezc-api/doclist",["require","exports","module","ace/edit_session","ace/undomanager","ace/lib/net","ace/ext/modelist"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ezc-api/doclist", arguments);
						...
						function initDoc(file, path, doc) {
						    ...
						    //파일경로 또는 이름의 확장자를 기준으로 편집기 모드 설정
						    var mode = modelist.getModeForPath(path, doc.name);
						...
						var docs = {
						  ...
						  "docs/c9search.c9search_results": "C9 Search Results", "docs/mel.mel": "MEL", 
						  "docs/Nix.nix":"Nix"
						 };
						*/
						...
						function prepareDocList(docs) {
						    ...
						    for (var path in docs) {
						        ...
						        doc.path = path;
						        doc.desc = doc.name; // .replace(/^(ace|docs|ezwel|build)\//,
														// "");
						        if (doc.desc.length > 18) {
						        	// doc.desc = doc.desc.slice(0, 7) + ".." +
									// doc.desc.slice(-9);
						        }
						
						        $.apiConfig.provider.fileCache[doc.name.toLowerCase()] = doc;
						        list.push(doc);
						    }
						...
						function loadDoc(name, callback) {
							$.logger.debug("+- loadDoc", arguments);
							
						    var doc = $.apiConfig.provider.fileCache[name.toLowerCase()];
						    ...
						    var path = doc.path;
						    /*
							 * var parts = path.split("/"); if (parts[0] == "docs") path =
							 * "ezwel/ezc-api/" + path; else if (parts[0] == "ace") path =
							 * "lib/" + path;
							 */
						    path = $.apiConfig.cont.ctxp + $.apiConfig.cont.txURL.findXML
						    
						    $.logger.debug("-doc", doc);
						    
						    net.get(path, {
						    	 storeFileDir: doc.prntName
						    	,storeFileName: doc.name
						    	,managedXml: doc.managedXml
						    }, function(x) {
						    	var response = JSON.parse(x);
						    	var out = {};
						    	if(response.code != "1000") {
						    		$.apiConfig.util.errorMessage(path, response);
						    		out = response.message;
						    	}
						    	else {
						    		out = response.xmlCont;
						    	}
						    	// $.logger.debug("[net.get Response]", x);
						    	
						        initDoc(out, path, doc);
						        callback(doc.session);
						    });
						}
						
						module.exports = {
						    fileCache: $.apiConfig.provider.fileCache,
						    initDoc: initDoc,
						    loadDoc: loadDoc,
						    prepareDocList: prepareDocList,
						    sort: sort
						};
						
						$.each(storeFile.fileList, function(i, data){
							
							// $.logger.warn(">>> $.apiConfig.provider.docs", $.apiConfig.provider.docs);
						...
						if (window.require && window.require.s) try {
							...
							$.logger.error("- window.require.s.contexts._.defined", e);
						}
						
						//문서 목록 exports
						for(var key in $.apiConfig.provider.docs) {
							module.exports[key] = sort(prepareDocList($.apiConfig.provider.docs[key]));
						}
				...
				define("ezc-api/layout",["require","exports","module","ace/lib/dom","ace/lib/event","ace/edit_session","ace/undomanager","ace/virtual_renderer","ace/editor","ace/multi_select","ace/theme/textmate"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ezc-api/layout", arguments);
				...
				define("ezc-api/util",["require","exports","module","ace/lib/dom","ace/lib/event","ace/edit_session","ace/undomanager","ace/virtual_renderer","ace/editor","ace/multi_select"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ezc-api/util", arguments);
						...
						exports.bindDropdown = function(id, callback, noInit) {
						    ...
						    var onChange = function() {
						    	$.logger.debug("-exports.bindDropdown onChange", el);
				...
				define("ace/ext/elastic_tabstops_lite",["require","exports","module","ace/editor","ace/config"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/ext/elastic_tabstops_lite", arguments);
				...
				define("ace/occur",["require","exports","module","ace/lib/oop","ace/range","ace/search","ace/edit_session","ace/search_highlight","ace/lib/dom"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/occur", arguments);
						...
						(function() {
						    ...
						    this.highlight = function(sess, regexp) {
						        ...
						        hl.setRegexp(regexp);
						        sess._emit("changeBackMarker"); // force highlight layer
																// redraw
						    };
				...
				define("ezc-api/token_tooltip",["require","exports","module","ace/lib/dom","ace/lib/oop","ace/lib/event","ace/range","ace/tooltip"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ezc-api/token_tooltip", arguments);
				...
				define("ace/split",["require","exports","module","ace/lib/oop","ace/lib/lang","ace/lib/event_emitter","ace/editor","ace/virtual_renderer","ace/edit_session"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/split", arguments);
						...
						(function(){
						    ...
						    this.$cloneSession = function(session) {
						    	$.logger.debug("## this.$cloneSession ##");
				...
				define("ace/ext/menu_tools/overlay_page",["require","exports","module","ace/lib/dom"], 
					function(require, exports, module) {
						'use strict';
						$.logger.warn("[define] ace/ext/menu_tools/overlay_page", arguments);
				...
				define("ace/ext/themelist",["require","exports","module"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/ext/themelist", arguments);
				...
				define("ace/ext/options",["require","exports","module","ace/ext/menu_tools/overlay_page","ace/lib/dom","ace/lib/oop","ace/lib/event_emitter","ace/ext/modelist","ace/ext/themelist"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/ext/options", arguments);
						...
						(function() {
						    ...
						    this.render = function() {
						        ...
						        buildDom(["div", {id: "controls"}, 
						            this.renderOptionGroup(optionGroups.Main),
						            ["ul", null, ["li", {colspan: 2},
						    ...
						    this.renderOptionGroup = function(group) {
						        return Object.keys(group).map(function(key, i) {
						        	// $.logger.debug("#renderOptionGroup => key: " +
									// key + ", i: " + i);
						    ...
						    this.renderOption = function(key, option) {
						        ...
						        return ["ul", {class: "ace_optionsMenuEntry"}, ["li",
				...
				define("ace/ext/statusbar",["require","exports","module","ace/lib/dom","ace/lib/lang"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/ext/statusbar", arguments);
				...
				define("ace/snippets",["require","exports","module","ace/lib/oop","ace/lib/event_emitter","ace/lib/lang","ace/range","ace/anchor","ace/keyboard/hash_handler","ace/tokenizer","ace/lib/dom","ace/editor"], 
						function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/snippets", arguments);
				...
				define("ace/ext/emmet",["require","exports","module","ace/keyboard/hash_handler","ace/editor","ace/snippets","ace/range","resources","resources","tabStops","resources","utils","actions","ace/config","ace/config"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/ext/emmet", arguments);
				...
				define("ace/autocomplete/popup",["require","exports","module","ace/virtual_renderer","ace/editor","ace/range","ace/lib/event","ace/lib/lang","ace/lib/dom"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/autocomplete/popup", arguments);
				...
				define("ace/ext/beautify",["require","exports","module","ace/token_iterator"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ace/ext/beautify", arguments);
				...
				define("ezc-api/ezwel",["require","exports","module","ace/lib/fixoldbrowsers","ace/ext/rtl","ace/multi_select","ezc-api/inline_editor","ezc-api/dev_util","ezc-api/file_drop","ace/config","ace/lib/dom","ace/lib/net","ace/lib/lang","ace/lib/useragent","ace/lib/event","ace/theme/textmate","ace/edit_session","ace/undomanager","ace/keyboard/hash_handler","ace/virtual_renderer","ace/editor","ace/ext/whitespace","ezc-api/doclist","ezc-api/layout","ezc-api/util","ace/ext/elastic_tabstops_lite","ace/incremental_search","ezc-api/token_tooltip","ace/config","ace/worker/worker_client","ace/split","ace/ext/options","ace/ext/statusbar","ace/ext/emmet","ace/placeholder","ace/snippets","ace/ext/language_tools","ace/ext/beautify","ace/keyboard/keybinding","ace/commands/command_manager"], 
					function(require, exports, module) {
						"use strict";
						$.logger.warn("[define] ezc-api/ezwel", arguments);
						...
						//cache split > $editors 스플릿될 2개의 에디터가 들어있음.
						$.apiConfig.provider.mainSplit = split;
						...
						commands.addCommand({
						    ...
						    exec: function(arg) {
						        ...
						        setTimeout(function(){
							    	...
							    	if(!$.gf_func.isValidateXml( session.getValue() )) {
							    		return false;
							    	}
							    	
						        	$.apiConfig.tx.saveXML( $.apiConfig.util.getUserId(), session.getValue(), function( response ){
						        		$.logger.debug("$.apiConfig.tx.saveXML callback", response);
						        		
						        		var storeFileName = response.storeFileName;
						        		
							        	localStorage.setItem( "saved_file:" + name, session.getValue() );
										// 에디터 하단 커멘드라인
										env.editor.cmdLine.setValue("saved complete "+ storeFileName + " (" + response.message + ")");
										
										//docs, fileCache, options doc, doc grp selectbox 변경 및 추가
										$.apiConfig.util.addDocument( module, storeFileName );
						...
						doclist.history = doclist[$.apiConfig.cont.firstData].map(function(doc) {
						    return doc.name;
						});
						...
						doclist.pickDocument = function(name) {
							$.logger.debug("#doclist.pickDocument", arguments);
							
						    doclist.loadDoc(name, function(session) {
						    	$.logger.debug("#doclist.loadDoc", arguments);
						        if (!session) {
						        	return;
						        }
						...
						// cache final function 
						$.apiConfig.provider.doclist = doclist;
						
						var OptionPanel = require("ace/ext/options").OptionPanel;
						var optionsPanel = new OptionPanel(env.editor);
						
						$.apiConfig.provider.optionsPanel = optionsPanel;
						...
						$.logger.debug("#optionsPanel", optionsPanel);
						...
						optionsPanel.setOption("doc", util.getOption("doc") || "XML");
						 ...
						 * net.loadScript("https://cloud9ide.github.io/emmet-core/emmet.js",
						 * function() { Emmet.setCore(window.emmet);
						...
						*/
						...
						event.addListener(document.documentElement, "keyup", function(e) {
						    if (e.keyCode === 18) // do not trigger browser menu on
													// windows
						        e.preventDefault();
						});
				...
				window.require(["ezc-api/ezwel"], function(m) {
			...
			,load: {
				applyLog: function( callback ) {
					$.apiConfig.tx.applyList( { storeType: "LOG" }, function(response) { 
						$("#applyLog").empty();
						$.each(response.fileList, function(idx, item){
							$("#applyLog").append($('<option>', {
								value: item.name,
								text : item.name
							}));							
						});
				...
				,backFile: function( callback ) {
					$.apiConfig.tx.applyList( { storeType: "BAKUP" }, function(response) { 
						$("#backupFile").empty();
						$.each(response.fileList, function(idx, item){
							$("#backupFile").append($('<option>', {
								value: item.name,
								text : item.name
							}));							
						});
			...
			,bind: function() {
				$("#applyXML").on("click", function( e ) {
					$.apiConfig.tx.applyXML( $.apiConfig.util.getUserId(), $("#controls #-doc").val(), function( response ){
						$.apiConfig.load.applyLog(function(){
							$.apiConfig.load.backFile();
						});
					});
				});
				
				$("#applyLog").on("change", function( e ) {
					
					$.apiConfig.tx.applyView( {
						 storeType: "LOG"
						,storeFileName: $(this).val()
					}, function( response ){
						
						if( $.apiConfig.provider.mainSplit ) {
							var editor = $.apiConfig.provider.mainSplit.getEditor(0);
							
							if(editor) {
								editor.setValue(response.xmlCont);
								editor.session.setMode("ace/mode/text");
								$("#controls #-doc").val("");
								$("#controls #-mode").val("ace/mode/text");
							}
						}
					...
					$.apiConfig.tx.applyView( {
						...
						if( $.apiConfig.provider.mainSplit ) {
							...
							if(editor) {
								...
								$("#controls #-doc").val("");
								$("#controls #-mode").val("ace/mode/xml");
							}
						}
...
$(document).ready(function() {
	...
	$.apiConfig.init(function( response ){
		$.apiConfig.define( response );
	});
